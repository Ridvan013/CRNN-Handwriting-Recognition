...
# OpenCV yuksek-volume uyarilari sustur (eksik image dosyalari icin)
try:
...
# ==========================
# PyTorch CRNN Model V3 (Even Larger: 4-layer BiLSTM, hidden=512)
# Total params: ~24M (V2: 15.46M, V1: 8.75M)
# ==========================
class CRNNModel(nn.Module):
    def __init__(self, img_height: int = 32, img_width: int = 128, num_classes: int = None):
        ...
        self.hidden = 512  # V3: was 384 in V2, 256 in V1

        # CNN Feature Extraction (UNCHANGED from v1 — asymmetric pooling is well-tuned)
        ...
        # V3 BiLSTM: 4 layers, hidden=512 (V2: 3 layers, hidden=384; V1: 2 layers, hidden=256)
        # Increased depth + width for higher visual modeling capacity.
        # Dropout=0.3 (slightly higher to prevent overfit on larger model).
        self.rnn = nn.LSTM(
            input_size=512,
            ...
            bidirectional=True,
            batch_first=False,
            dropout=0.3,  # active because num_layers>1
        )

        # Classifier: 2*hidden = 768 -> num_classes
    ...
    def forward(self, x):
        ...
        # 3-layer BiLSTM
        lstm_out, _ = self.rnn(rnn_input)  # [W', B, 2*hidden]
...
class IAMDataset(Dataset):
    ...
    def _apply_gpu_augmentation(self, img_tensor):
        """GPU'da genisletilmis augmentation uygula.
        Tum operasyonlar tensor uzerinde, CPU bottleneck yaratmaz.
        Hedef: handwriting style invariance + writer-disjoint generalization.
        """
        import random

        # 1. Random rotation (p=0.6, +/- 7 degree - v2 stronger)
        if random.random() < 0.6:
            angle = random.uniform(-7, 7)
            img_tensor = TF.rotate(img_tensor.unsqueeze(0), angle,
                                   interpolation=TF.InterpolationMode.BILINEAR,
                                   fill=1.0).squeeze(0)

        # 2. Random affine translation (p=0.6, +/- 5%)
        if random.random() < 0.6:
            h, w = img_tensor.shape[-2:]
            dx = random.uniform(-0.05, 0.05) * w
            dy = random.uniform(-0.05, 0.05) * h
            img_tensor = TF.affine(img_tensor.unsqueeze(0), angle=0,
                                   translate=[dx, dy], scale=1.0, shear=[0.0, 0.0],
                                   interpolation=TF.InterpolationMode.BILINEAR,
                                   fill=1.0).squeeze(0)

        # 3. Random scale (p=0.3, 0.9-1.1x) - handles variable word sizes
        if random.random() < 0.3:
            scale = random.uniform(0.9, 1.1)
            img_tensor = TF.affine(img_tensor.unsqueeze(0), angle=0,
                                   translate=[0, 0], scale=scale, shear=[0.0, 0.0],
                                   interpolation=TF.InterpolationMode.BILINEAR,
                                   fill=1.0).squeeze(0)

        # 4. Random shear (p=0.3, +/- 5 degree) - italics simulation
        if random.random() < 0.3:
            shear_x = random.uniform(-5, 5)
            img_tensor = TF.affine(img_tensor.unsqueeze(0), angle=0,
                                   translate=[0, 0], scale=1.0, shear=[shear_x, 0.0],
                                   interpolation=TF.InterpolationMode.BILINEAR,
                                   fill=1.0).squeeze(0)

        # 5. Brightness + contrast (p=0.6, 0.85-1.15x)
        if random.random() < 0.6:
        ...
        # 6. Gaussian noise (p=0.4, sigma=0.03)
        if random.random() < 0.4:
            noise = torch.randn_like(img_tensor) * 0.03
            img_tensor = torch.clamp(img_tensor + noise, 0.0, 1.0)

        # 7. Gamma correction (p=0.4, 0.8-1.2) - ink intensity variation
        if random.random() < 0.4:
            gamma = random.uniform(0.8, 1.2)
            img_tensor = TF.adjust_gamma(img_tensor, gamma)

        # 8. Random erasing / cutout (p=0.3, small patch) - ink stain simulation
        if random.random() < 0.3:
            h, w = img_tensor.shape[-2:]
            patch_h = random.randint(max(1, h // 16), max(2, h // 6))
            patch_w = random.randint(max(1, w // 16), max(2, w // 6))
            y = random.randint(0, max(0, h - patch_h))
            x = random.randint(0, max(0, w - patch_w))
            # Fill with white (background color) - simulates ink fading / paper bleed
            img_tensor[..., y:y + patch_h, x:x + patch_w] = 1.0
...
def beam_search_decode(log_probs, input_lengths, beam_width=10):
    ...
    Maintains top-k partial hypotheses ranked by cumulative log-probability.
    Per-step expansion limited to top-(beam_width*2) chars for efficiency.

    log_probs: (sequence_length, batch_size, num_classes), time-major
    input_lengths: (batch_size,)
    Returns: list of decoded index lists (one per batch sample).
    """
    seq_len, batch_size, num_classes = log_probs.shape
    blank_index = len(CHAR_LIST)
    results = []

    # Move to CPU once for the per-step Python loop; faster than per-op .item().
    lp = log_probs.detach().cpu()

    for batch_idx in range(batch_size):
        seq_len_actual = int(input_lengths[batch_idx])
        pred = lp[:seq_len_actual, batch_idx, :]  # (T, C)

        # Each beam: (sequence_tuple, last_emitted_char, cumulative_log_prob)
        beams = [((), None, 0.0)]

        for t in range(seq_len_actual):
            # Limit branching to top-k characters per step (huge speed gain)
            top_k = min(beam_width * 2, num_classes)
            ...
            for seq_tuple, last_char, score in beams:
                for char_log_p, char_idx in zip(top_probs, top_idxs):
                    ...
                    # Merge beams that collapse to same (sequence, last_char)
                    if key in new_beams:
                        # log-sum-exp merge
                        prev = new_beams[key]
                        if new_score > prev:
            ...
            # Materialize and prune to top-k
...
class CRNNTrainer:
    def __init__(self, model, device='auto', use_mixed_precision=True, trigram_lm=None):
        ...
        # V3 Optimizer (AdamW with weight decay for better regularization)
        self.optimizer = optim.AdamW(
            self.model.parameters(),
            lr=7e-4,        # V3: slightly higher peak LR for bigger model
            weight_decay=1e-5,
        )

        # V3 Learning rate scheduler: linear warmup (5 epochs) + cosine decay
        WARMUP_EPOCHS = 5
        TOTAL_EPOCHS = int(os.environ.get("CRNN_EPOCHS", "60"))
        def lr_lambda(epoch):
            if epoch < WARMUP_EPOCHS:
                return (epoch + 1) / WARMUP_EPOCHS
            # Cosine decay from peak to ~1% of peak over remaining epochs
            progress = (epoch - WARMUP_EPOCHS) / max(1, TOTAL_EPOCHS - WARMUP_EPOCHS)
            return 0.01 + 0.99 * 0.5 * (1.0 + math.cos(math.pi * progress))
        self.scheduler = LambdaLR(self.optimizer, lr_lambda)
        self._use_warm_cosine = True  # Flag so train loop uses .step() per-epoch unconditionally
        ...
        self.early_stopping_patience = 15  # V3: more patience for cosine schedule
    ...
    def train(self, train_loader, val_loader, epochs=50):
        ...
        for epoch in range(epochs):
            ...
            # V3 LR scheduling: warmup + cosine takes only epoch number (no metric).
            # ReduceLROnPlateau (if used) takes val_loss.
            if getattr(self, '_use_warm_cosine', False):
                self.scheduler.step()
            else:
...
# ==========================
RECORDS_COUNT = None  # Tum IAM veri setini kullan (None = sinirsiz, tum veriyi yukle)
AUGMENTATION_FACTOR = 5  # Her goruntu icin 5 farkli versiyon (hibrit augmentation icin)
EPOCHS = int(os.environ.get("CRNN_EPOCHS", "60"))  # V3: 60 epochs with cosine LR (5 warmup + 55 decay)
BATCH_SIZE = int(os.environ.get("CRNN_BATCH", "128"))  # CRNN_BATCH env ile override edilebilir
...
# ==========================
# Aachen-RWTH writer-disjoint split
# ==========================
AACHEN_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "aachen_splits", "splits")

def _load_aachen_forms(name):
    p = os.path.join(AACHEN_DIR, f"{name}.uttlist")
    if not os.path.exists(p):
        raise FileNotFoundError(f"Aachen split file not found: {p}")
    with open(p) as f:
        return set(line.strip() for line in f if line.strip())

AACHEN_TRAIN = _load_aachen_forms("train")
AACHEN_VAL   = _load_aachen_forms("validation")
AACHEN_TEST  = _load_aachen_forms("test")
print(f"[Aachen] Loaded forms: train={len(AACHEN_TRAIN)}, val={len(AACHEN_VAL)}, test={len(AACHEN_TEST)}")

skipped_not_in_aachen = 0
...
for idx, line in enumerate(lines):
    ...
    # FAST PATH: Aachen membership check BEFORE expensive imread.
    # form_id = first two hyphen-separated tokens of the word id
    # (e.g. "a01-000u-00-01" -> "a01-000u").
    # Bu kontrol sayesinde user-added ve gecersiz kayitlarda
    # disk I/O ve OpenCV uyarisi olusmuyor.
    form_id = "-".join(word_id.split("-")[:2])
    # Skip obvious non-IAM forms (e.g., user-added). IAM forms have format
    # like "a01-000u" (letter + 2 digits + dash + 3 digits + optional letter).
    if not form_id or "-" not in form_id or form_id.startswith("user"):
        skipped_not_in_aachen += 1
        continue
    if form_id in AACHEN_TRAIN:
        ...
        # Form not in any Aachen partition (340 forms, ~7050 words).
        # Bu formlar ne val'da ne test'tedir => writer-disjoint kuralini bozmaz.
        # Training'e ekleyelim (+%22 ekstra veri).
        target_bucket = "train"
    ...
    if target_bucket == "train":
        train_images.append(img)
        train_labels.append(lab)
    elif target_bucket == "val":
...
print(f"  Skipped (not in Aachen):    {skipped_not_in_aachen:,}")
...
test_dataset = IAMDataset(test_images, test_labels, augmentation_factor=1, is_training=False, device=DEVICE)
...
# Test loader (Aachen test set) - bagimsiz held-out degerlendirme icin
test_loader = DataLoader(
    ...
    num_workers=0,
    pin_memory=False,
    drop_last=False,  # Son batch'i ATLAMA - test setinin tum orneklerini logla
...
# Create trainer
# Aachen-disjoint trigram LM: yalnizca Aachen training transcriptions'dan kuruluyor,
# boylece test set kelimeleri LM vocabulary'sine sizmiyor (paper'in lexical hygiene'i icin)
trigram_lm = None
aachen_train_words_file = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    "aachen_splits", "train_words.txt",
)
lm_source = aachen_train_words_file if os.path.exists(aachen_train_words_file) else WORDS_FILE
if lm_source and os.path.exists(lm_source):
    try:
        from trigram_lm import TrigramLanguageModel
        print(f"[Info] Trigram LM yukleniyor (Aachen-disjoint): {lm_source}")
        trigram_lm = TrigramLanguageModel(lm_source)
    except ImportError:
        print("[Warning] trigram_lm.py bulunamadi, validation Trigram'siz yapilacak.")
...
# Model Kaydetme
# ==========================
model_dir = "Model_aachen_v3"  # v3: 4-layer BiLSTM hidden=512, ~24M params + cosine LR + label smoothing
...
def create_detailed_analysis_csv(model, val_loader, model_dir, device='cuda', trigram_lm=None):
    """Test sonuclarini detayli analiz eden CSV dosyasi olusturur (GPU'da).
    DORT decoder konfigurasyonu loglar:
      - Raw_Predicted_Text      : greedy decode only
      - Predicted_Text          : greedy + trigram correction (FULL PIPELINE - main number)
      - Beam_Predicted_Text     : beam search (k=10) only
      - Beam_Trigram_Text       : beam search + trigram correction
    Bu sayede McNemar testleri:
      - Greedy vs Greedy+Trigram (LM katkisi)
      - Greedy vs Beam (decoder katkisi)
      - Greedy vs Beam+Trigram (her ikisinin birlesik etkisi)
    hepsi tek CSV'den uretilebilir.
    """
    try:
        import pandas as pd

        print("Creating detailed analysis CSV (greedy + beam, both with/without trigram)...")
        ...
        with torch.no_grad():
            for images, labels in val_loader:
                ...
                all_greedy.extend(greedy_preds)

                # Beam search decode (slower, k=10) - sadece test eval'de calistirilir
                beam_preds = beam_search_decode(log_probs, input_lengths, beam_width=10)
                all_beam.extend(beam_preds)
        ...
        raw_greedy_texts = [indices_to_text(pred) for pred in all_greedy]
        raw_beam_texts = [indices_to_text(pred) for pred in all_beam]
        true_texts = [dense_to_text(target) for target in all_targets]

        # Trigram correction on both greedy and beam outputs
        if trigram_lm:
            print("Applying Trigram correction to greedy and beam outputs...")
            greedy_corrected = [trigram_lm.correct_word(t) for t in raw_greedy_texts]
            beam_corrected = [trigram_lm.correct_word(t) for t in raw_beam_texts]
        else:
            greedy_corrected = list(raw_greedy_texts)
            beam_corrected = list(raw_beam_texts)

        # Analysis rows
        analysis_data = []
        for i, (gr_raw, gr_lm, bm_raw, bm_lm, true) in enumerate(
                zip(raw_greedy_texts, greedy_corrected, raw_beam_texts, beam_corrected, true_texts)):
            is_correct = gr_lm.strip() == true.strip()
            char_errors = levenshtein_distance(gr_lm, true)
            word_length = len(true.strip())
            char_accuracy = 1 - (char_errors / max(1, len(true)))

            error_type = "None"
            if not is_correct:
                if len(gr_lm) < len(true):
                    error_type = "Under-prediction"
                elif len(gr_lm) > len(true):
            ...
            analysis_data.append({
                'Sample_ID': i,
                'True_Text': true,
                'Raw_Predicted_Text': gr_raw,         # greedy only
                'Predicted_Text': gr_lm,              # greedy + trigram (main pipeline)
                'Beam_Predicted_Text': bm_raw,        # beam k=10 only
                'Beam_Trigram_Text': bm_lm,           # beam k=10 + trigram
                'Is_Correct': is_correct,
                'Word_Length': word_length,
                'Character_Errors': char_errors,
                'Character_Accuracy': round(char_accuracy, 4),
                'Error_Type': error_type,
                'Length_Difference': len(gr_lm) - len(true),
            })
        # Make pred_texts alias for downstream visualization (old code expects it)
        pred_texts = greedy_corrected
...
# Analiz ve gorsellestirme islemlerini calistir
# Aachen pipeline: en iyi checkpoint val set'inde secilir, FINAL sayilar TEST set'inden raporlanir
if len(test_images) > 0 and len(train_images) > 0:
    # Once best validation checkpoint'i geri yukle (val WA'ye gore secilmis)
    if os.path.exists(best_wa_path):
        print(f"\n[Aachen] Reloading best_model_wa.pth for held-out TEST evaluation...")
        ckpt = torch.load(best_wa_path, map_location=DEVICE)
        if isinstance(ckpt, dict) and 'model_state_dict' in ckpt:
            model.load_state_dict(ckpt['model_state_dict'])
        else:
            model.load_state_dict(ckpt)
        print("[Aachen] Best checkpoint loaded.")

    # FINAL HELD-OUT TEST evaluation - bu sayilar makaleye gider
    print(f"\n{'=' * 63}")
    print("[Aachen] Running held-out TEST set evaluation (this populates the paper numbers)")
    print(f"{'=' * 63}")
    analysis_df, pred_texts, true_texts, all_images = create_detailed_analysis_csv(
        model, test_loader, model_dir, device=DEVICE, trigram_lm=trigram_lm
    )

    # Tahmin gorsellestirmelerini olustur (test setinden)
