...
as well, measured where each can be isolated: a lexical corrector taken
apart into the lexicon, the ranking rule and the place it is applied, whose
effects differ by an order of magnitude; an
augmentation ablation with a positive control and three seeds, which shows
the effect under study to be smaller than the variation between two runs of
the same configuration; and a parameterization in common use that turns out
to leave the image unchanged. None of the three depends on our particular
network, and each is reported so that it can be re-derived from the released
