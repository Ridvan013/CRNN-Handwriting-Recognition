...
% revision, scored in fp32 with cuDNN deterministic algorithms.
...
between writers. We study word-level recognition on the IAM
...
that rescores candidates with a Kneser--Ney trigram and decodes each
text line jointly, conditioning every word on the recognizer's own
output for both neighbours. Two questions are examined under identical
conditions. The first concerns the post-corrector, usually reported as
one dictionary or language-model step.
Decomposing it reveals four ingredients with different jobs: lexicon
coverage decides the sign of the correction
...
points); an external corpus adds 0.6 and line context another 0.6,
moving accuracy from \rawwa\ to \ourwa\ without raising the character
error rate. The second question is whether writing-aware
...
\begin{itemlist}
  ...
  \item \textbf{A controlled augmentation ablation with a positive
        ...
        costs \anchordelta\,pp at $p=1\times10^{-30}$. These results are consistent with a saturation
...
among those 2\,938. A candidate $c$ at edit distance $d$ from the hypothesis is
scored as
\begin{equation}
...
greedy hypotheses; two separate processes produce byte-identical
...
sign every time, never below the threshold. With one seed per
configuration we therefore treat the difference as unresolved rather
than as an established gain. Because the configurations are nested,
...
validation and is the configuration of \ours; on the other five
optical models of Table~\ref{tab:main} it gains $0.32$--$0.44$\,pp
...
more but by leaving alone the words it would have broken. These are
...
$p=1\times10^{-30}$, 2.7 points of word accuracy. The
...
largest effect ($+0.32$\,pp, $p=0.132$; $+0.44$\,pp, $p=0.025$ under
the left-to-right corrector) stays below the threshold and changes
with the post-corrector. What we observe is consistent with
...
trigram over the unigram prior is equally stable: $+0.84$ to
$+0.95$\,pp on each of the six optical models, with $\alpha=7$ selected
on validation for all of them; whole-line decoding then adds $+0.32$
