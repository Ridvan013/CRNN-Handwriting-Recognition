...
% Sources: results/ablation_viterbi.json (Table 2; Table 4 rows 10-13),
% results/ablation_viterbi_seeds.json (Table 3),
% results/ablation_lexicon5_all.json (Table 4 rows 1-5),
% results/ablation_trigram.json (Table 4 rows 5-9, oracle),
...
about a point and line context half of one, moving accuracy from \rawwa\ to
...
\anchordelta\ points, and three seeds turn that gap into noise.
\end{abstract}
...
words. Against the baseline, \ours\ is \deltawa\ higher at $p=0.050$,
...
Table~\ref{tab:main} \deltawa\ at $p=0.050$. The sign never changes and
...
at $p=0.050$, $0.026$ and $1\times10^{-3}$ --- yet two of them point in
...
\textbf{\ours} (all)   & 83.80 & \textbf{7.96} & [83.29,\,84.30] & $+0.39$ & $+0.19$ & 0.050 & 84.77 (70) \\
\botrule
\end{tabular}}
\end{table}
...
Seed & CRNN-B & \ours & $\Delta$ & $p$ & CER: CRNN-B / \ours \\
     & WA (\%) & WA (\%) & (pp) & & (\%) \\
\colrule
42 & 83.41 & 83.80 & $+0.39$ & 0.050 & 8.27 / 7.96 \\
...
runs the authors' own implementation on the same CRNN-LX probabilities with
the same three lexicons.
...
\begin{table}[th]
\tbl{Word beam search on the \emph{same} CRNN-LX probabilities (the
authors' implementation,\cite{scheidl2018wbs} beam width 25,
...
whose largest effect (\deltawa, $p=0.050$) does not survive the threshold,
...
prior by a trigram with line context about half of one; the trigram earns
...
the 3.6\% of hypotheses that
fall outside the lexicon, and feeding it the reference transcriptions of the
neighbouring crops instead of its own outputs adds $0.06$\,pp.
...
The corrector effects are stable across optical models where we measured
them on all of them: with the word-list lexicon, forced replacement against
...
rule about a point, the line context half of one, and imposing the lexicon
