...
% revision. Sources: results/ablation_trigram_all.json (Table 2),
% results/ablation_lexicon5_all.json + results/ablation_trigram.json (Table 3),
% results/paper_stats_trigram.json (per-word counts), results/brown_leakage.json,
% per-word predictions in results/preds_trigram/
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\newcommand{\ourwa}{81.66\%}           % CRNN-LX: KN trigram + line context
\newcommand{\ourci}{[81.13\%, 82.19\%]}
\newcommand{\ourcer}{8.58\%}
\newcommand{\uniwa}{80.74\%}           % same model, unigram prior, no context
\newcommand{\unicer}{9.09\%}
\newcommand{\rawwa}{78.83\%}
\newcommand{\rawcer}{8.60\%}
\newcommand{\baselinewa}{81.22\%}      % CRNN-B, trigram corrector (Table 2 reference)
\newcommand{\deltawa}{0.44\,pp}        % CRNN-LX minus CRNN-B, trigram corrector
\newcommand{\anchordelta}{2.41}        % conventional pipeline removed, pp (trigram corrector)
\newcommand{\nsamples}{20{,}310}
...
\begin{abstract}
Recognizing handwritten words is difficult because handwriting varies
strongly between writers. We study word-level recognition on the IAM
database under the Aachen writer-disjoint protocol, on all \nsamples\
test words. Our system, \ours, is a convolutional-recurrent network
trained from scratch and followed by a lexicon-aware post-corrector
that rescores candidates with a Kneser--Ney trigram whose context is
the recognizer's own output for the preceding words on the line. Two
questions are examined under identical conditions. The first concerns
the post-corrector, usually reported as a single dictionary or
language-model step. Decomposing it reveals four ingredients with
different jobs: lexicon coverage decides the sign of the correction
(the 7\,K-word training vocabulary lowers accuracy by 2.4 points, a
239\,K-type general English list raises it by 0.7); the frequency
prior that ranks candidates supplies most of the magnitude (1.2
points); an external corpus adds 0.6 and line context 0.3, moving
accuracy from \rawwa\ to \ourwa\ without raising the character error
rate. The second question is whether writing-aware
augmentation (elastic, morphological and wide photometric transforms)
helps. Under a single-seed setting, six configurations differing only
in augmentation show no statistically detectable gain from these
transforms, whereas removing the conventional pipeline beneath them
costs \anchordelta\ points.
\end{abstract}
...
the proposed system that model is a Kneser--Ney trigram whose context
is the recognizer's own output for the preceding words on the line. We
...
\begin{itemlist}
  \item \textbf{An anatomy of lexicon-assisted correction.} Holding the
        ...
        $0.6$\,pp through the corpus and $0.3$\,pp through the line
        context, and brings the character error rate back below that of
        lexicon-free decoding. The complete corrector delivers
        $+2.8$\,pp, from \rawwa\ to \ourwa.
  \item \textbf{A controlled augmentation ablation with a positive
        ...
        costs \anchordelta\,pp at $p=8\times10^{-27}$. These results are consistent with a saturation
effect rather than with an inability of the experimental setup to
detect augmentation-related improvements.
...
is then corrected against a 239\,K-type lexicon, the candidates being
ranked by a Kneser--Ney trigram whose context is the system's own
output for the preceding crops of the same line. The two outlined
...
post-corrector of Section~\ref{sec:trigram} with the trigram and line
context, and is the configuration we propose. The suffixes denote the
...
best validation word accuracy is evaluated once on the test set.
...
(16.1\%) fall outside the extended lexicon, and 2\,938 of them have at
least one candidate within that distance and are replaced; the
remaining words pass through unchanged, so everything the corrector can
gain or lose is decided among those 2\,938. A candidate $c$ at edit distance $d$ from the hypothesis is
scored as
\begin{equation}
...
penalty selected on the validation partition from $\{3,5,7,10\}$. The
lexicon and the language model are the two variables of interest, and
Section~\ref{sec:res-lex} varies them independently with the optical
model held fixed.
...
84.8\% of the test tokens and the extended lexicon 94.0\%; the
...
47\,997 IAM training words, with $\alpha=5$. It is the corrector of an
...
on the IAM training lines (47\,997 tokens) and, in the proposed
...
available at test time. The context $h_1 h_2$ in Eq.~(\ref{eq:score})
is the system's \emph{own output} for the two preceding word crops of
the same line --- never their reference transcription. The words of a
line are corrected from left to right, each output becoming the context
of the next; the context is reset at the start of a line and wherever a
preceding crop is missing from the split (crops whose segmentation IAM
flags as not \texttt{ok}). On the test set 88.5\% of the words have a
preceding crop. The optical model still sees one isolated crop at a
time; only the post-corrector reads across the line, and the
no-context figure is reported alongside for the setting in which words
...
reported below comes from evaluation passes in single precision, in
which the six optical models and all post-corrections are scored
against identical greedy hypotheses; two independent runs of a pass
agree on all \nsamples\ hypotheses. Mixed-precision inference, by
contrast, is not bit-reproducible and shifted 2 words between runs
(0.01\,pp). The post-corrector is deterministic: candidates are
...
in which lines are processed. A numerical self-test confirms that the
trigram's conditional distributions sum to one over the 271\,303-type
vocabulary for seen and unseen contexts. Code, split
...
(81.22 to 78.81\%) and $1.62$\,pp of character error rate; 1\,289
words are recognized only by the baseline against 800 only by the
anchor, and the exact McNemar test rejects equality at
$p=8\times10^{-27}$. Without a lexicon the same comparison is wider
still, 78.76 against 74.33\% ($-4.43$\,pp), so part of the weaker
optical model is masked by the post-corrector.

Under the reported single-seed setting, the proposed transforms do not
provide a statistically detectable improvement. The five
configurations that keep the conventional pipeline reach
81.22--81.66\%, and none differs from the baseline at the $p<0.01$
threshold of Section~\ref{sec:experiments}. The full scheme (\ours) is
nominally the best, \deltawa\ above the baseline, which is within one
confidence-interval half-width. Its $p$-value of $0.025$ would pass a
conventional $0.05$ threshold, but not one corrected for the five
comparisons made against the baseline ($0.05/5=0.01$); elastic
perturbation alone gives $p=0.055$, morphological perturbation
$0.189$ and the widened photometric range $0.760$. The comparison is
also sensitive to the post-corrector: scored with the unigram prior of
Table~\ref{tab:lexicon}, the same two optical models differ by
$0.38$\,pp at $p=0.060$. With one seed per configuration we therefore
treat the difference as unresolved rather than as an established gain.
Because the configurations are nested, Table~\ref{tab:main} reports
two differences: $\Delta_{\mathrm{B}}$ against the baseline CRNN-B,
and $\Delta_{\mathrm{P}}$ against the $+$wide photometric configuration
on which elastic and morphological perturbation are built, which
isolates each transform. By $\Delta_{\mathrm{P}}$, elastic perturbation
adds $0.32$\,pp, morphological perturbation $0.19$\,pp and the two
together $0.38$\,pp; the widened photometric range itself adds
$0.06$\,pp, which is why $\Delta_{\mathrm{B}}$ is larger by that
amount ($+0.38$, $+0.26$ and $+0.44$\,pp). On the per-word level
these five disagree on 3\,202 words (15.8\%), yet the disagreements
largely cancel: for the baseline--\ours\ pair, 747 words are
recognized only by the baseline and 837 only by \ours. Among configurations that
...
post-corrector (239\,K lexicon, Kneser--Ney trigram on IAM$+$Brown with
line context; the edit penalty, selected on validation for each model
separately, is $\alpha=7$ for all six). $\Delta_{\mathrm{B}}$ is the difference in WA to
...
\colrule
no augmentation        & 78.81 & 10.53 & [78.25,\,79.37] & $-2.41$ & ---     & $8\!\times\!10^{-27}$ & 82.57 (38) \\
CRNN-B (baseline)      & 81.22 & 8.91  & [80.68,\,81.75] & (ref.)  & ---     & (ref.) & 84.61 (87) \\
$+$ wide photometric   & 81.29 & 8.84  & [80.74,\,81.82] & $+0.06$ & (ref.)  & 0.760  & 84.34 (76) \\
$+$ elastic            & 81.60 & 8.60  & [81.06,\,82.13] & $+0.38$ & $+0.32$ & 0.055  & 84.69 (77) \\
$+$ morphological      & 81.48 & 8.68  & [80.94,\,82.01] & $+0.26$ & $+0.19$ & 0.189  & 84.58 (84) \\
\textbf{\ours} (all)   & \textbf{81.66} & \textbf{8.58} & [81.13,\,82.19] & $+0.44$ & $+0.38$ & 0.025 & 84.77 (70) \\
\botrule
\end{tabular}}
\end{table}
...
set a little less tightly, as a regularizer should), but that
...
isolate the frequency prior at a fixed lexicon, and rows 5--9 isolate
the corpus and the context at a fixed lexicon.
...
$+0.34$\,pp, reaching \ourwa\ ($p=1\times10^{-35}$ against the unigram
prior; 220 words gained against 32 lost). Decomposed in the other
order --- context first, corpus second --- the shares are $+0.10$ and
$+0.83$\,pp, so the two ingredients are not additive: context is worth
little until the corpus is large enough to have seen it. Two
diagnostics bound the result. Replacing the system's own outputs by the
reference transcriptions of the neighbouring crops (an oracle, not a
reportable system) gives 81.73\%, only $0.06$\,pp more, so almost
nothing is lost to the recognizer's own errors in the context. And the
edit penalty is not what carries the result: $\alpha=7$ was selected on
the validation set, but every value tried ($3$, $5$, $7$, $10$) gives
between 81.43 and 81.66\% on the test set, all above the unigram
prior.
...
language model, corpus and context vary one at a time. KN1/KN3:
interpolated Kneser--Ney unigram/trigram; ``$+$ctx'': the trigram is
given the system's own outputs for the two preceding crops of the
line. Edit penalty $\alpha=5$ for the add-one prior; for the KN models
$\alpha$ is selected on the validation set (5 for row 6, 7 for rows
7--9).\label{tab:lexicon}}
...
extended lexicon $+$ KN3$+$ctx            & KN3$+$ctx (IAM)           & 239\,126 & 80.84 & 9.07 & [80.29,\,81.37] \\
extended lexicon $+$ KN1                  & KN1 (IAM$+$Brown)         & 239\,126 & 81.32 & 8.73 & [80.78,\,81.85] \\
\textbf{extended lexicon $+$ KN3$+$ctx} (\ours) & KN3$+$ctx (IAM$+$Brown) & 239\,126 & \textbf{81.66} & \textbf{8.58} & [81.13,\,82.19] \\
\botrule
\end{tabular}}
\end{table}
...
alone, the unigram prior, the Brown corpus and the line context (white
labels: increment over the previous bar).}
\label{fig:lexdecomp}
\alttext{Two panels. Left: line chart of the change in word accuracy against lexicon size, with and without the frequency prior, relative to lexicon-free decoding. Right: four bars showing the cumulative gain of edit distance, unigram prior, Brown corpus and line context on the 239K lexicon.}
\end{figure}
...
the lexicon-free level (8.58\%). Lexicon-assisted word accuracy and
...
5.5\,pp with it. \ours\ remains below the later attention-based
systems: 0.9\,pp below Kang et al.~\cite{kang2018convolve}, 2.4\,pp
below the candidate-fusion model of Kang et
al.~\cite{kang2021candidate} and 2.9\,pp below
...
gap is 0.9\,pp wider (1.8, 3.4 and 3.9\,pp), and against our
lexicon-free figure 2.8\,pp wider. The gap is larger at character
level (5.79--6.88\% against our 8.58--8.60\%), consistent with the
...
\textbf{\ours} (ours)                         & 2026 & 239\,K $+$ trigram, line context & 81.66 & 8.58 \\
\colrule
...
\ours\ misrecognizes 3\,724 of the \nsamples\ test words. Only 275 of
them (7.4\%) differ from the reference in letter case alone; the
remaining errors are genuine. At character level the aligned errors
consist of 5\,050 substitutions, 1\,563 deletions and 413 insertions,
...
(231 cases in both directions), \texttt{s}$\leftrightarrow$\texttt{r}
(142), \texttt{t}$\leftrightarrow$\texttt{l} (95),
\texttt{n}$\leftrightarrow$\texttt{m} (92),
\texttt{n}$\leftrightarrow$\texttt{r} (89),
\texttt{e}$\leftrightarrow$\texttt{a} (88).
Case substitutions account for 8.0\% of all substitutions.
Of the post-corrector's decisions, 2\,938 hypotheses (14.5\%) are
replaced: 1\,076 wrong hypotheses become correct and 501 correct ones,
valid words absent from the lexicon, become wrong. The unigram prior
replaces exactly the same hypotheses and breaks the same 501 words ---
the lexicon, not the ranking, decides which words are touched --- but
repairs only 888. These are
errors of shape, not of vocabulary: a whole-word lexicon can repair them
...
noisy to see anything, and with a single seed per configuration that
objection has force. But the same test, the same data and the same seed
detect the removal of the conventional pipeline at
$p=8\times10^{-27}$, 2.4 points of word accuracy. The
apparatus is not blind to augmentation; at a single seed it cannot
resolve \emph{these} transforms on \emph{this} training set, whose
largest effect ($+0.44$\,pp, $p=0.025$) stays below the threshold and
changes with the post-corrector. What we observe is consistent with
...
regularization that the network genuinely needs. Second, among the
...
worth 2.4\,pp --- but it does show that further transforms must be
...
estimate of which of two candidates is the commoner English word. The
ceiling is close: the corrector only ever touches the 16\% of
hypotheses that fall outside the lexicon, and with reference
transcriptions as context the same corrector gains a further
$0.06$\,pp only. What remains after correction is therefore not a
ranking problem. The character error rate confirms the division of
labour: the unigram corrector buys word accuracy at the price of
character accuracy (9.09\% against 8.60\% lexicon-free), because a wrong
whole-word substitution turns a near miss into several character
errors. The trigram replaces exactly the same 2\,938 hypotheses but
picks the right candidate for 1\,076 of them instead of 888
(Section~\ref{sec:res-err}), and ends below the lexicon-free value
(8.58\%).
...
to 4.5\,pp without context and 5.5\,pp with it. Sueiras et al. also report lexicon-constrained variants of
...
about augmentation: under our single-seed setting, Table~\ref{tab:main}
shows no measurable contribution of the proposed transforms to the
margin.
...
word accuracy, 1.58\,pp below \ours\ (0.66\,pp below its no-context
...
0.1--0.4\,pp are below what a single seed can resolve, which is why we
...
grows as the optical model weakens. The gain of the trigram with line
context over the unigram prior is equally stable: $+0.84$ to
$+0.95$\,pp on each of the six optical models, with $\alpha=7$ selected
on validation for all of them. These effects are, however, measured
...
comparison. The extended lexicon is a general English word list and
...
was selected on the validation writers and applied once to the test
set, and the result holds for all four values tried.
...
system is 0.9--2.9\,pp below the later attention-based systems listed
in Table~\ref{tab:priorwork}, and 1.8--3.9\,pp below them when the
words are treated as isolated. Under controlled, single-seed
conditions, elastic deformation, morphological perturbation and a
widened photometric range produce no statistically detectable change
in these figures, while removing the conventional augmentation
pipeline beneath them costs \anchordelta\,pp --- a pattern consistent
with the benefit of augmentation saturating before the transforms we
proposed; a widely used elastic parameterization, moreover, does not
...
adds the last third of a point and returns the character error rate to
the lexicon-free level.
...
and that an ablation reporting no effect should include the
configuration in which the effect is known to exist; that lexicon
...
address the uncovered 6\% of tokens, and let the context flow both
ways along the line, since the corrector now reads only the words to
the left of each crop.
