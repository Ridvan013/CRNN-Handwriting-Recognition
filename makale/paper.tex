...
\begin{itemlist}
  ...
  \item \textbf{A controlled augmentation ablation with a positive
        ...
        $+0.66$\,pp) and two of the three per-seed tests are significant
        in opposite directions, while the mean difference is
        $+0.20\pm0.58$\,pp. These results are consistent with a
...
$0.75$ and an additive floor over the model's vocabulary --- the union of the
three lexicons and of the corpus, 271\,303 types --- so that every candidate
keeps a positive probability; an unseen trigram context
...
differences reach $p<0.05$ and they disagree in sign.\label{tab:seeds}}
...
Ranking adds about a point and context about another half. Within a lexicon,
...
while gaining word accuracy. The corpus lexicon is the first configuration
that improves both: \ourcer\ against \rawcer, with
4.97\,pp of word accuracy
gained. Word accuracy and character error rate therefore have to be reported
...
\textbf{\ours} (ours)                         & 2026 & 57\,K corpus $+$ trigram, line context & 83.80 & \textbf{7.96} \\
...
whose largest effect (\deltawa, $p=0.049$) does not survive the threshold,
moves with the post-corrector, and changes sign with the seed
...
word-list lexicon, more than the ranking rule and the line context together
contribute on either lexicon. It is also the smaller of the two, so what
...
1.2\,pp behind the corpus lexicon under the same rule. The practical
...
whole-line decoder, which has two options and a wider penalty grid, was
selected against on the corpus lexicon, and the variant that was not
selected (real-word substitution) reaches almost the same test accuracy by a
different route --- evidence that the selection is not finely tuned to the
test set, but also that the validation partition cannot separate such
variants.
...
accuracy by five points up and two and a half down, and its parts are not
equally important: the lexicon is worth
2.1\,pp when it is
