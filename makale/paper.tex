...
greedy hypotheses and reports WA (CER) in per cent; the Wilson half-width is
about 0.55\,pp throughout. Columns: the candidate within the edit bound is
...
from margins of this size. Our rows in this table are there to situate
the system, not to compete with it: the comparisons this paper controls
are those of Tables~\ref{tab:main}--\ref{tab:lexicon}, in which one
ingredient changes at a time and the optical hypotheses are identical.
...
recognizes each word in isolation. Bold marks the best value in each
column.\label{tab:priorwork}}
{\begin{tabular}{llccc}
\toprule
...
similar lower-case glyphs --- \texttt{a}$\leftrightarrow$\texttt{o} (215), \texttt{r}$\leftrightarrow$\texttt{s} (139), \texttt{l}$\leftrightarrow$\texttt{t} (95), \texttt{n}$\leftrightarrow$\texttt{r} (83), \texttt{m}$\leftrightarrow$\texttt{n} (80), \texttt{a}$\leftrightarrow$\texttt{e} (77) --- counting both directions.
