...
Two recent systems are close to ours in task, metric and model family.
Dutta et al.~\cite{dutta2018improving} report an improved
CNN--recurrent neural network (RNN) hybrid on isolated IAM words with
a 22.86\% word error rate ($\approx$77.14\% accuracy). Rajesh et
al.~\cite{rajesh2022hwrcnet} propose HWRCNet, a CNN--BiLSTM operating
in the JPEG discrete cosine transform (DCT) domain, reaching 80.08\%
strict word accuracy on uncompressed images; we note that their
headline figure of 89.05\% is \emph{word accuracy with flexibility} of
two edit operations rather than strict accuracy. On the decoder side,
word beam search (WBS)~\cite{scheidl2018wbs} injects a dictionary
constraint directly into CTC beam search. We instead apply the lexical
knowledge \emph{after} greedy decoding, which lets us swap the lexicon
without retraining or re-decoding and therefore isolate its effect.

\paragraph{Positioning.}
Both word-level CRNN baselines above are evaluated on random
partitions of IAM that do not enforce writer disjointness (HWRCNet
uses a 95:5 ratio), whereas all our figures are obtained on the
stricter Aachen protocol. Comparisons across the two protocols are
indicative only; the controlled comparisons of this paper are those
...
Table~\ref{tab:priorwork} places \ours\ next to six published
word-level systems. The upper block lists results that, to the best of
our knowledge, were obtained under the same Aachen writer-disjoint
protocol. \ours\ exceeds the early sequence-to-sequence recogniser of
Sueiras et al.~\cite{sueiras2018offline} by 4.5\,pp but remains below
the attention-based systems: 1.8\,pp below Kang et
...
knowledge from 14.4\,M scene-text images. The gap is larger at
character level (5.79--6.88\% against our 9.09\%), consistent with the
...
Where a cited system does not apply a closed test-time lexicon, our
lexicon-free figure (\rawwa, CER \rawcer) is the more conservative
reference. The lower block reports two CRNN-family systems measured on
random, writer-dependent partitions; \ours\ is above both, but the
protocols differ and we draw no conclusion from margins of this size.
...
Kass and Vats~\cite{kass2022attentionhtr}     & 2022 & (\textbf{84.60}) & 6.50 \\
\oursnolm\ (ours, lexicon-free)               & ---  & 78.82 & 8.60 \\
\textbf{\ours} (ours, extended lexicon)       & ---  & 80.73 & 9.09 \\
\midrule
...
\subsection{Comparison with HWRCNet}

HWRCNet~\cite{rajesh2022hwrcnet} is the closest published system in
construction --- a CNN, a bidirectional LSTM and a CTC layer --- and
reports 80.08\% strict word accuracy, 0.65\,pp below \ours. The two
differ in capacity (five convolutional layers and 256-unit LSTMs
against seven blocks and 512 units), in the use of a post-correction
lexicon, and in protocol (a random 95:5 split against writer-disjoint
evaluation). Given Table~\ref{tab:main}, we attribute none of the
difference to augmentation, and given the protocol mismatch we refrain
from attributing it to anything in particular; the comparison is
...
image data. Under controlled conditions, elastic deformation,
