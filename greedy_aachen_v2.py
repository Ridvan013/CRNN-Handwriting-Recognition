...
# ==========================
# PyTorch CRNN Model V2 (Larger: 3-layer BiLSTM, hidden=384)
# Total params: ~16M (was 8.75M)
# ==========================
class CRNNModel(nn.Module):
    def __init__(self, img_height: int = 32, img_width: int = 128, num_classes: int = None):
        ...
        # V2 BiLSTM: 3 layers, hidden=384 (was 2 layers, hidden=256)
        # Each layer takes 768-dim input (bidi prev output) except first (512 from CNN)
        # IMPORTANT: dropout=0.2 in nn.LSTM with num_layers>1 IS effective (between layers).
        # So we use a SINGLE nn.LSTM with num_layers=3 to actually get inter-layer dropout.
        self.rnn = nn.LSTM(
            input_size=512,
            ...
            bidirectional=True,
            batch_first=False,
            dropout=0.2,  # active because num_layers>1
        )
...
EPOCHS = int(os.environ.get("CRNN_EPOCHS", "50"))  # CRNN_EPOCHS env ile override edilebilir (dry-run icin)
...
# Model Kaydetme
# ==========================
model_dir = "Model_aachen_v2"  # v2: 3-layer BiLSTM hidden=384, ~16M params
